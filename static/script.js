      ...
      success: function (response) {
        ...
        hospitalInfo.forEach((hospital) => {
          ...
          suggestionItem.innerHTML = `
                         ${hospital.HospitalID} - 
                         <span class="${hospital.Status.toLowerCase()}">${
            hospital.Status
          }</span>
                     `;
  ...
  function createEditableFields(rowData, headers) {
    ...
    document
      ...
      .addEventListener("click", () => {
        ...
        const headerTexts = Array.from(editableheaders).map(
  ...
  function updateDataInAWS(updatedRowData) {
    ...
    $.ajax({
      ...
      error: function (xhr, status, error) {
        ...
        showUploadStatus(
          "Error during the upload. Please try again later.",
          "error"
        );
      },
    });
